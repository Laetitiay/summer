...
bool Shape::hasIntersection(Shape & other)
{
    for (int i = 0; i < other.size(); ++i)
    {
        for (int j = 0; j < size(); ++j)
        {
            ...
            bool p3_sign = signbit(Shape::threePointsCalc(other.points[i], other.points[i], other.points[(j + 1) % other.size()]));
            bool p4_sign = signbit(Shape::threePointsCalc(other.points[(i + 1) % other.size()], other.points[j], other.points[(j + 1) % size()]));
